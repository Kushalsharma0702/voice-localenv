...
class CallManagementService:
    ...
    async def trigger_single_call(self, customer_id: str, websocket_id: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if exotel_response['success']:
                ...
                db_call_session = create_call_session(session, call_data)
                
                # Update Redis session with actual call SID
                self.redis_manager.update_call_session(temp_call_id, {'call_sid': call_sid})
                
                # ✅ Normalize to CALL_IN_PROGRESS instead of RINGING
                self.redis_manager.update_call_status(call_sid, CallStatus.CALL_IN_PROGRESS, "Call initiated successfully")
                update_call_status(session, call_sid, CallStatus.CALL_IN_PROGRESS, "Call initiated successfully", 
                                {'exotel_response': exotel_response['response']})
                
                # Update customer call status to 'call_in_progress'
                update_customer_call_status(session, customer_id, CallStatus.CALL_IN_PROGRESS)
                ...
                self.redis_manager.update_call_status(temp_call_id, CallStatus.CALL_FAILED, f"Failed to initiate call: {exotel_response['error']}")
                
                # Update customer call status to 'failed'
                update_customer_call_status(session, customer_id, CallStatus.CALL_FAILED)
    ...
    #updated bulk call with batch processing
    async def trigger_bulk_calls(self, customer_ids: List[str], websocket_id: str = None) -> Dict[str, Any]:
        """Trigger calls in batches using trigger_single_call so statuses behave like single calls"""
        ...
        for i in range(0, len(customer_ids), BATCH_SIZE):
            ...
            # Call each customer in batch via trigger_single_call (handles statuses + broadcasts)
            batch_tasks = [
                asyncio.create_task(self.trigger_single_call(customer_id, websocket_id))
                for customer_id in batch_ids
            ]
